...
async def show_form(flow: FlowHandler, step: str, first_time: bool, data=None):
    ...
    dict_.update({
        ...
        vol.Optional(CONF_MONITORED_CONDITIONS,
                     default=data.get(CONF_TRIGGER_TIME, list(SENSOR_TYPES.keys()))): config_validation.multi_select(
            SENSOR_TYPES),
        vol.Optional(CONF_TRIGGER_TIME,
                     default=data.get(CONF_TRIGGER_TIME, {"minutes": 2})): cv.positive_time_period_dict,
        vol.Optional(CONF_SCAN_INTERVAL,
                     default=data.get(CONF_SCAN_INTERVAL, {"minutes": 1})): cv.positive_time_period_dict
    })
    return flow.async_show_form(
        step_id=step, data_schema=vol.Schema(dict_)
    )
...
class ConfigFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):  # pylint: disable=dangerous-default-value
        ...
        if user_input is not None:
            if user_input[CONF_HOST] != "":
                ...
                fix_time_in_dict(user_input, CONF_SCAN_INTERVAL)
    ...
    def async_create_entry(self, *, title: str, data: Mapping[str, Any], description: Optional[str] = None,
                           description_placeholders: Optional[dict] = None,
                           options: Optional[Mapping[str, Any]] = None) -> data_entry_flow.FlowResult:
        return super().async_create_entry(title=title, data=data, description=description,
                                          description_placeholders=description_placeholders, options=options)
    ...
    async def async_step_init(self, user_input=None):
        ...
        if user_input is None:
            ...
            if CONF_PASSWORD in user_input:
                ...
                fix_time_in_dict(self._data, CONF_SCAN_INTERVAL)
            return self.async_create_entry(title=self._data[CONF_HOST], data=self._data)
